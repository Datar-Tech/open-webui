...
3.  **If Navigating to an Existing Chat:**
...
### External Tool Server Integration Flow (via Frontend Direct Call)
This flow describes when the frontend directly calls a pre-configured external tool server.
```mermaid
sequenceDiagram
    participant U as User
    participant FE as Frontend (+layout.svelte, executeTool)
    participant TS as External Tool Server

    U->>FE: Interacts with UI element that triggers a tool
    FE->>FE: Retrieves Tool Server URL and auth from settings
    FE->>TS: HTTP Request (e.g., POST /tool-action-endpoint with params and auth header)
    TS-->>FE: HTTP Response (JSON, text, etc.)
    FE->>U: Displays result from Tool Server or uses it in chat
```
*Note: The backend (`BE`) is primarily involved in storing tool server configurations. The actual execution request often goes directly from frontend to the tool server.*
